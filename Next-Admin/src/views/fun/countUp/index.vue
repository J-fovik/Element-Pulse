				...
				<el-col :sm="6" class="mb15" v-for="(v, k) in state.topCardItemList" :key="k">
					<div class="countup-card-item countup-card-item-box" :style="{ background: `var(${v.color})` }">
						<div class="countup-card-item-flex" ref="topCardItemRefs">
							...
							<div class="countup-card-item-title-num pb6">{{ v.titleNum }}</div>
				...
				<el-col :sm="6" class="mb15" v-for="(v, k) in state.topCardItemList" :key="k">
					<div class="countup-card-item countup-card-item-box" :style="{ background: `var(${v.color})` }">
						<div class="countup-card-item-flex" ref="topCardItemRefs">
							<div class="countup-card-item-title pb3">{{ v.title }}</div>
							<div class="pb6">
								<countUpCom :value="v.titleNum"></countUpCom>
							</div>
							<div class="countup-card-item-tip pb3">{{ v.tip }}</div>
							<countUpCom :value="v.tipNum"></countUpCom>
						</div>
						<i :class="v.icon" :style="{ color: v.iconColor }"></i>
					</div>
				</el-col>
...
import countUpCom from '/@/components/countUp/index.vue';
// 定义变量内容
...
const initNumCountUp = () => {
	nextTick(() => {
		topCardItemRefs.value.forEach((v: HTMLDivElement) => {
			const num = v.querySelector('.countup-card-item-title-num')?.textContent;
			if (num) {
				new CountUp(v.querySelector('.countup-card-item-title-num') as HTMLDivElement, parseFloat(num)).start();
			}
