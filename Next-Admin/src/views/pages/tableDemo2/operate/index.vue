<template>
	<div class="layout-pd">
		...
		<el-card class="footerCard flex justify-end items-end">
			<el-space>
				<el-button v-if="!route.query.isReadOnly" type="primary" :loading="activeKey === 'okLoading'" @click="validateData(formRef)"
					>确定</el-button
				>
...
<script setup lang="ts" name="pagesTableDemo2Operate">
...
const rules = {
	...
	startDate: {
		...
		validator: (rule: any, value: any, callback: any) => {
			...
			if (form.value.endDate && dayjs(value).isAfter(dayjs(form.value.endDate))) callback(new Error('开始时间不能大于结束时间'));
			callback();
		},
		trigger: 'blur',
	},
	endDate: {
		...
		validator: (rule: any, value: any, callback: any) => {
			if (!value) callback(new Error('请选择开始时间'));
			if (form.value.startDate && dayjs(value).isBefore(dayjs(form.value.endDate))) callback(new Error('结束时间不能小于开始时间'));
			callback();
		},
		trigger: 'blur',
	},
