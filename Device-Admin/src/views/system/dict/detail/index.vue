			...
			<template #operateRight>
				<el-space>
					<el-button
						...
						:icon="CirclePlus"
						@click="
							setSelectRow({ dictType: curDictType.dictType }, () =>
			...
			<template #table>
				...
				>
					<template #dictLabel="{ row }">
						<el-tag
							:type="
								['default', '', 'primary'].includes(row.listClass)
									? 'primary'
									: row.listClass
							"
...
const handleDelete = (row: any) => {
	let title = row?.dictId ? row.dictLabel : '已选中的数据项';
	...
	ElMessageBox.confirm(`是否确认删除${title}？`, '警告', {
		...
		async () => {
			const { res, err } = await curryingRequestUrl(
				{
					url: URL.SYSTEM.HANDLE_DICT_DATA + '/' + dictIds,
					method: 'delete',
				},
				{
...
const {
	...
	const { res, err } = await curryingRequestUrl({
		...
		params: {
			...form.value,
			dictType: curDictType.value.dictType,
...
const getDetail = async () => {
	const { res, err } = await curryingRequestUrl({
		url: URL.SYSTEM.HANDLE_DICT + '/' + route.query.id,
		method: 'get',
	});
	if (err) return;
	curDictType.value = res?.data;
	initData();
};
/* 查询详情和编辑时才调用 */
if (route.query.id) {
