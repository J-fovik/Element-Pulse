					...
					<el-row :gutter="24">
						...
						<el-col :span="4">
							<el-form-item label="描述">
								<el-input v-model.trim="form.msg" placeholder="描述" clearable />
							</el-form-item>
						</el-col>
						<el-col :span="4">
							<el-form-item label="描述">
...
// 批量删除操作
const handleDelete = () => {
	ElMessageBox.confirm('是否确认删除已选登录日志数据项？', '警告', {
		confirmButtonText: '确定',
		cancelButtonText: '取消',
		type: 'warning',
	}).then(
		async () => {
			const { res, err } = await curryingRequestUrl(
				{
					url:
						URL.SYSTEM.HANDLE_LOGIN +
						'/' +
						tableBaseOptions.selectedKeys.map((o) => o.infoId).join(','),
					method: 'delete',
				},
				{
					before: () => setActiveKey('loading'),
